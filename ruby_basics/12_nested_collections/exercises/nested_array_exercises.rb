...
def count_empty_seats(chart)
  ...
  x = (chart.reduce([]) do |total, row|
    
    row.select do |seat|
      total.push(seat) if seat == nil
    end
...
def find_favorite(array_of_hash_objects)
  ...
  # array_of_hash_objects.each do |lang|
  #   if lang[:is_my_favorite?] == false
  #     array_of_hash_objects.delete(lang)
  #   end
  # end

  # return array_of_hash_objects[0]

  # array_of_hash_objects.all? do |language|
  #   language[:is_my_favorite?] == false
  #   return nil
  # end

  if array_of_hash_objects.all? {|language| language[:is_my_favorite?] == false}
    return nil
  else
    array_of_hash_objects.each do |lang|
      if lang[:is_my_favorite?] == false
...
# rspec spec/nested_array_exercises_spec.rb